...
class UnknownColumnInSchemaException : public std::exception {
    std::string message;
public:
    explicit UnknownColumnInSchemaException(const std::string& column)
        : message("Unknown column in schema: " + column) {}
    const char* what() const noexcept override {
...
public:
    void add_column(const std::string& name, ColumnType type) {
        if (columns.contains(name)) throw DuplicateColumnException();
...
// CSV Import
// -----------------------------
void load_csv(const std::string& csv, Table& table,
              const std::unordered_map<std::string, ColumnType, std::hash<std::string>, std::equal_to<>>& schema) {
    ...
    while (std::getline(header_ss, col, ',')) {
        ...
        if (!schema.contains(col)) {
            throw UnknownColumnInSchemaException(col);
        }
